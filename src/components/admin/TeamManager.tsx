...
export default function TeamManager() {
  ...
  return <div className="space-y-7">
    ...
    <div className="overflow-x-auto rounded-xl border border-neutral-800"><table className="w-full min-w-[760px] text-left"><thead className="bg-neutral-900 text-neutral-400"><tr><th className="p-4">Pessoa</th><th>Cargos</th><th>Comissão</th><th>Perfil</th><th>Ação</th></tr></thead><tbody>{members.map(member=>{const roles=[...new Set([member.role,...(member.roles||[])])];return <tr key={member.id} className="border-t border-neutral-800"><td className="p-4"><b>{member.full_name||"Sem nome"}</b><p className="text-sm text-neutral-500">{member.email}</p></td><td>{roles.join(" · ")}</td><td>{roles.includes("photographer")?`${Number(member.commission_rate).toFixed(2)}%`:"—"}</td><td>{member.public_profile?<Link className="text-green-400 underline" href={`/fotografos/${member.id}`} target="_blank">Publicado</Link>:"Privado"}</td><td><button type="button" onClick={()=>edit(member)} className="rounded bg-neutral-800 px-3 py-2">Editar</button></td></tr>})}</tbody></table></div>
  </div>;
}
