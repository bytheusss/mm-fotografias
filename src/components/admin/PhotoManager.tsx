...
export default function PhotoManager({
  ...
  async function setPrice(price: string) { if (!selected.size || price === "") return; const response = await fetch("/api/admin/photos/batch", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: [...selected], action: "price", price: Number(price.replace(",", ".")) }) }); const data=await response.json().catch(()=>({})); setMessage(response.ok ? `Preço aplicado em ${selected.size} fotos.` : data.error || "Erro ao definir preço."); if (response.ok) await load(page); }
  ...
  return (
    <div className="mt-12">
      ...
      {selected.size > 0 && <form onSubmit={event => { event.preventDefault(); const form=event.currentTarget,value=new FormData(form).get("price");if(typeof value==="string")void setPrice(value);form.reset(); }} className="mb-5 flex flex-col gap-3 rounded-lg bg-neutral-900 p-3 sm:flex-row sm:items-center"><label className="text-sm">Preço das selecionadas:</label><input name="price" required inputMode="decimal" placeholder="R$ 15,00" className="min-w-0 flex-1 rounded bg-black p-2"/><button className="rounded bg-neutral-700 px-4 py-2 font-bold">Aplicar preço</button></form>}
