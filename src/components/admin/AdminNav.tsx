...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex h-dvh max-h-dvh max-w-[88vw] flex-col border-r border-neutral-800 bg-neutral-950 pt-[calc(5rem+env(safe-area-inset-top))] transition-all duration-200 ${collapsed ? "w-20" : "w-64"} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      ...
      {!collapsed&&<div className="mt-10 px-3"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar no painel…" className="w-full rounded-lg border border-neutral-800 bg-black px-3 py-2 text-sm text-white outline-none focus:border-red-700"/></div>}<nav className={`${collapsed ? "mt-10" : "mt-3"} min-h-0 flex-1 overflow-y-auto overscroll-contain pb-4 [scrollbar-width:thin] ${collapsed ? "px-2" : "px-3"}`}>{groups.map(([group, links]) => {const visible=links.filter(([,label])=>label.toLowerCase().includes(query.toLowerCase()));return visible.length?<section key={group} className="mb-4">{!collapsed && <p className="mb-1 px-4 text-[10px] font-bold tracking-[.18em] text-neutral-600">{group}</p>}<div className="space-y-1">{visible.map(([icon, label, href]) => { const active = href === "/admin" ? pathname === href : pathname.startsWith(href); return <Link title={collapsed ? label : undefined} onClick={() => setMobileOpen(false)} key={href} href={href} className={`flex items-center rounded-lg py-2.5 text-sm transition ${collapsed ? "justify-center px-2" : "gap-3 px-4"} ${active ? "bg-red-700 font-bold text-white" : "text-neutral-400 hover:bg-neutral-900 hover:text-white"}`}><span aria-hidden="true" className="w-5 text-center text-base">{icon}</span>{!collapsed && <span>{label}</span>}</Link>; })}</div></section>:null})}</nav>
