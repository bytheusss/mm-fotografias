...
const groups = [
  ...
  ["CONTEÚDO", [["▣", "Eventos", "/admin/events"], ["↑", "Upload", "/admin/upload"], ["♟", "Fotógrafos", "/admin/collaborators"], ["▥", "Relatório da equipe", "/admin/photographer-report"], ["♜", "Equipe", "/admin/team"]]],
  ["VENDAS", [["▤", "Pedidos", "/admin/orders"], ["◴", "Carrinhos", "/admin/abandoned-carts"], ["↗", "Financeiro", "/admin/finance"]]],
  ["MARKETING", [["$", "Pacotes", "/admin/pricing"], ["%", "Cupons", "/admin/coupons"], ["★", "Promoção", "/admin/promotion"]]],
  ["SISTEMA", [["♲", "Lixeira", "/admin/trash"], ["◈", "LGPD", "/admin/privacy"], ["≡", "Auditoria", "/admin/audit"], ["⚙", "Integrações", "/admin/integrations"]]],
] as const;
...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex max-h-dvh flex-col border-r border-neutral-800 bg-neutral-950 pt-24 transition-all duration-200 ${collapsed ? "w-20" : "w-64"} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
    ...
    <div className={`min-w-0 transition-[padding] duration-200 ${collapsed ? "lg:pl-20" : "lg:pl-64"}`}>{children}</div>
  </>;
}
