...
export function GalleryLightbox({ photos, index, onIndexChange, onClose }: { photos: EventPhoto[]; index: number; onIndexChange: (index: number) => void; onClose: () => void }) {
  ...
  useEffect(() => { if (!photo?.eventId) return; const sessionKey = localStorage.getItem("mm-session") || crypto.randomUUID(); localStorage.setItem("mm-session", sessionKey); void fetch("/api/interactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ photoId: photo.id, eventId: photo.eventId, kind: "view", sessionKey }) }); }, [photo?.id, photo?.eventId]);
  ...
  return <div className="fixed inset-0 z-[100] flex bg-black/95" role="dialog" aria-modal="true" aria-label={`Visualização da foto ${photo.numero}`} onClick={onClose}>
    ...
    <div className="grid h-full w-full grid-rows-[minmax(0,1fr)_auto] md:grid-cols-[1fr_310px] md:grid-rows-1" onClick={event => event.stopPropagation()}>
      ...
      <aside className="relative z-20 flex flex-col border-t border-neutral-800 bg-neutral-950 p-5 text-white md:border-l md:border-t-0 md:p-6">
        ...
        <div className="mt-auto grid gap-3 pt-5"><button type="button" onClick={() => { toggleFavorite(photo); if (photo.eventId) void fetch("/api/interactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ photoId: photo.id, eventId: photo.eventId, kind: "favorite", sessionKey: localStorage.getItem("mm-session") }) }); }} className="rounded-lg border border-neutral-700 px-4 py-3 font-bold hover:bg-neutral-800">{favorite ? "♥ Remover dos favoritos" : "♡ Adicionar aos favoritos"}</button><button type="button" onClick={() => { addToCart(photo); if (photo.eventId) void fetch("/api/interactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ photoId: photo.id, eventId: photo.eventId, kind: "cart", sessionKey: localStorage.getItem("mm-session") }) }); }} disabled={added || photo.salesPaused} className="rounded-lg bg-red-600 px-4 py-3 font-bold hover:bg-red-700 disabled:bg-neutral-700 disabled:text-neutral-400">{photo.salesPaused ? "Vendas pausadas" : added ? "Adicionada ao carrinho ✓" : "Adicionar ao carrinho"}</button><a href="/carrinho" className="rounded-lg bg-neutral-800 px-4 py-3 text-center font-bold hover:bg-neutral-700">Ver carrinho ({items.length})</a></div>
      </aside>
    </div>
  </div>;
}
