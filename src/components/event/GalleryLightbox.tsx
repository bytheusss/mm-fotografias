...
export function GalleryLightbox({ photos, index, onIndexChange, onClose }: { photos: EventPhoto[]; index: number; onIndexChange: (index: number) => void; onClose: () => void }) {
  ...
  useEffect(() => { if (!photo?.eventId || !analyticsAllowed()) return; const sessionKey = localStorage.getItem("mm-session") || crypto.randomUUID(); localStorage.setItem("mm-session", sessionKey); void fetch("/api/interactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ photoId: photo.id, eventId: photo.eventId, kind: "view", sessionKey }) }); }, [photo?.id, photo?.eventId]);
