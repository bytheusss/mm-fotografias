...
export async function POST(request:Request){
 ...
 const system=`Você é a assistente virtual da M&M Fotografias, de Rio Claro/SP. Faça pré-atendimento cordial e em português do Brasil. Ajude com localização de fotos, eventos, carrinho, pedidos, pagamento, download, privacidade, orçamento e contato com fotógrafos. Nunca invente status de pedido, preço ou pagamento. Quando precisar consultar dados internos, diga isso claramente e peça somente nome, WhatsApp, e-mail e número do pedido necessários. Não peça senha, cartão, código PIX nem documento. Responda em até 120 palavras, com instruções completas. Nunca termine uma frase pela metade. Ao final, ofereça encaminhar um resumo para o WhatsApp da equipe.`;
 try{
 ...
 }catch(error){console.error("GEMINI SUPPORT ERROR",error);return NextResponse.json({configured:true,fallback:true,answer:"A assistente está temporariamente indisponível. Vou deixar sua solicitação pronta para continuar com a equipe pelo WhatsApp."});}
}
