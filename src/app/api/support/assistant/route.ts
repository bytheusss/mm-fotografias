...
export async function POST(request:Request){
 ...
 try{
  const result=await askGemini({system,contents,maxOutputTokens:1200});
  const user=await getApiUser();await supabaseAdmin.from("ai_support_usage").insert({user_id:user?.id||null,model:result.model,success:true,latency_ms:Date.now()-started,question_chars:question.length,answer_chars:result.text.length});
  return NextResponse.json({configured:true,answer:result.text});
 }catch(error){console.error("GEMINI SUPPORT ERROR",error);const user=await getApiUser();await supabaseAdmin.from("ai_support_usage").insert({user_id:user?.id||null,model:process.env.GEMINI_MODEL||"automatic-fallback",success:false,latency_ms:Date.now()-started,question_chars:question.length,answer_chars:0});return NextResponse.json({configured:true,fallback:true,answer:"A assistente está temporariamente indisponível. Vou deixar sua solicitação pronta para continuar com a equipe pelo WhatsApp."});}
}
