...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { error: insertError } = await supabaseAdmin.from("photos").insert({ event_id: id, number, title: `Foto ${padded}`, slug: `${event.slug}-${padded}`, original_path: `originals/${finalPath}`, preview_path: `previews/${finalPath}`, thumbnail_path: `thumbnails/${finalPath}`, price: 15, status: "available", featured: false, checksum: String(checksum || "") || null });
