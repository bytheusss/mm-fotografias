...
export async function POST(request:Request){
 ...
 const context=JSON.stringify({orders,events,tasks,expenses,tickets}).slice(0,50000);
 try{
  const result=await askGemini({system:"Você é o analista operacional interno da M&M Fotografias. Responda em português do Brasil usando exclusivamente os dados fornecidos. Não invente números. Diferencie faturamento de lucro e informe quando faltarem dados. Seja direto, com no máximo 250 palavras e sugestões práticas. Nunca execute ações.",contents:[{role:"user",parts:[{text:`DADOS:\n${context}\n\nPERGUNTA: ${question}`}]}],maxOutputTokens:1800});
