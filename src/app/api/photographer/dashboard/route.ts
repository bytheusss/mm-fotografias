...
export async function GET(){
  ...
  const [{data:profile},{data:photos},{data:orders},{data:payouts}]=await Promise.all([
    supabaseAdmin.from("profiles").select("full_name,commission_rate,public_profile").eq("id",id).single(),
  ...
  return NextResponse.json({profile,stats:{uploaded:owned.size,sold,revenue,commission,paid,pending:Math.max(0,commission-paid)},payouts:payouts||[]});
}
