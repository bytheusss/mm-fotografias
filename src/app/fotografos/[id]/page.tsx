...
export default async function PhotographerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: legacyTestimonials }, { data: verifiedReviews }] = await Promise.all([supabaseAdmin.from("photographer_testimonials").select("id,client_name,content,rating").eq("photographer_id", id).eq("published", true).order("created_at", { ascending: false }).limit(6),supabaseAdmin.from("purchase_reviews").select("id,customer_name,comment,rating").eq("photographer_id", id).eq("published", true).order("created_at", { ascending: false }).limit(6)]);
  const testimonials=[...(verifiedReviews||[]).map(item=>({id:item.id,client_name:item.customer_name,content:item.comment||"Excelente trabalho.",rating:item.rating,verified:true})),...(legacyTestimonials||[]).map(item=>({...item,verified:false}))].slice(0,6);
  ...
  return <main className="min-h-screen bg-black px-4 pb-24 pt-32 text-white sm:px-6"><div className="mx-auto max-w-6xl">
    ...
    {!!testimonials.length&&<><h2 className="mb-5 mt-10 text-2xl font-black">O que dizem sobre este profissional</h2><div className="grid gap-4 md:grid-cols-2">{testimonials.map(item=><blockquote key={item.id} className="rounded-xl border border-neutral-800 bg-neutral-900 p-5"><p className="text-yellow-400">{"★".repeat(item.rating)}</p><p className="mt-2 text-neutral-300">“{item.content}”</p><footer className="mt-3 flex flex-wrap items-center gap-2 font-bold">{item.client_name}{item.verified&&<span className="rounded-full bg-green-950 px-2 py-1 text-xs text-green-400">✓ Compra verificada</span>}</footer></blockquote>)}</div></>}
