...
export default async function PhotographerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: testimonials } = await supabaseAdmin.from("photographer_testimonials").select("id,client_name,content,rating").eq("photographer_id", id).eq("published", true).order("created_at", { ascending: false }).limit(6);
