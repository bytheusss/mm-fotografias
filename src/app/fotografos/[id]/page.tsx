...
export default async function PhotographerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: profile }, { data: assignments }, { count }] = await Promise.all([
    supabaseAdmin.from("profiles").select("id,full_name,bio,instagram_handle,avatar_url,role,roles").eq("id", id).contains("roles", ["photographer"]).eq("public_profile", true).maybeSingle(),
