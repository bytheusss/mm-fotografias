...
export const getAllEvents = cache(async (): Promise<Event[]> => {
  ...
  return (visible || []).map(event => ({ id: event.id, slug: event.slug, name: event.name, city: event.city, date: new Date(event.event_date).toLocaleDateString("pt-BR"), photoCount: event.total_photos || 0, image: event.cover_image, shareMessage: event.share_message, basePrice: Number(event.base_price), accessMode: event.access_mode, salesPaused: event.sales_paused, passwordVersion: Number(event.password_version || 1) }));
});
